 ...
 */
 ...
 /* global variables to store PLL constants values */
 static uint32_t global_uint32PLLQPres = 0;
 static uint32_t global_uint32PLLPPres = 0;
 static uint32_t global_uint32PLLNMult = 0; 
 static uint32_t global_uint32PLLMPres = 0;
 
 /* global variables to store bus prescaler values */
 static uint8_t global_uint8AHBPres = 0;
 static uint8_t global_uint8APB1Pres = 0;
 static uint8_t global_uint8APB2Pres = 0;
 ...
 SRV_enuErrorStatus_t MRCC_enuConfigPLL(MRCC_structPLLConfig_t* ptr_structPLLConfig)
 {
     ...
     else
     {
         ...
         else
         {
             ...
             global_uint32PLLQPres = ptr_structPLLConfig->Q_Prescaler;
             global_uint32PLLPPres = ptr_structPLLConfig->P_Prescaler;
             global_uint32PLLNMult = ptr_structPLLConfig->N_Multiplier;
             global_uint32PLLMPres = ptr_structPLLConfig->M_Prescaler;
         }
     }
 ...
 SRV_enuErrorStatus_t MRCC_enuSetBusClkPrescaler(MRCC_structBusClkPrescaler_t* ptr_structBusClkPrescalers)
 {
     ...
     else
     {
         ...
         /* save bus prescaler values to retrieve them for bus clk calculations */
         global_uint8AHBPres =  ptr_structBusClkPrescalers->AHB_Prescaler;
         global_uint8APB1Pres = ptr_structBusClkPrescalers->APB_LowSpeedPrescaler;
         global_uint8APB2Pres = ptr_structBusClkPrescalers->APB_HighSpeedPrescaler;
     }
...
SRV_enuErrorStatus_t MRCC_enuGetBusFreq(MRCC_enuSystemBuses_t copy_enuBus, uint64_t* ptr_uint64BusClkFreq)
{
    SRV_enuErrorStatus_t ret_enuErrorStatus = ALL_OK;
    volatile RCC_Registers_t* RCC = (volatile RCC_Registers_t*)RCC_BASE_ADDRESS;
    uint64_t local_uint64SysClkSrc = 0;
 
    if (IS_NULL_PTR(ptr_uint64BusClkFreq))
    {
        ret_enuErrorStatus = NULL_PTR;
    }
    else if ((copy_enuBus != SYS_BUS_AHB1) && (copy_enuBus != SYS_BUS_AHB2) && (copy_enuBus != SYS_BUS_APB1) && (copy_enuBus != SYS_BUS_APB2))
    {
        ret_enuErrorStatus = RCC_INV_BUS;
    }
    else 
    {
        /* get sysclk source + its freq */
        MRCC_enuGetSysClkSrc(&local_uint64SysClkSrc);
        switch (local_uint64SysClkSrc)
        {
            case MRCC_SYS_CLK_HSI:
                local_uint64SysClkSrc = RCC_HSI_CLK_HZ;
            break;
 
            case MRCC_SYS_CLK_HSE:
                local_uint64SysClkSrc = RCC_HSE_CLK_HZ;
            break;
 
            case MRCC_SYS_CLK_PLL:
                local_uint64SysClkSrc = RCC_HSI_CLK_HZ * (global_uint32PLLNMult / global_uint32PLLPPres) * (1 / global_uint32PLLMPres) * (1 / global_uint32PLLQPres);
            break;
        }
 
        /* calculate AHB1 bus clk */
        if (global_uint8AHBPres == MRCC_AHB_DIV_BY_1) {}
        else if (global_uint8AHBPres == MRCC_AHB_DIV_BY_2)   { local_uint64SysClkSrc /=   2; }
        else if (global_uint8AHBPres == MRCC_AHB_DIV_BY_4)   { local_uint64SysClkSrc /=   4; }
        else if (global_uint8AHBPres == MRCC_AHB_DIV_BY_8)   { local_uint64SysClkSrc /=   8; }
        else if (global_uint8AHBPres == MRCC_AHB_DIV_BY_16)  { local_uint64SysClkSrc /=  16; }
        else if (global_uint8AHBPres == MRCC_AHB_DIV_BY_64)  { local_uint64SysClkSrc /=  64; }
        else if (global_uint8AHBPres == MRCC_AHB_DIV_BY_128) { local_uint64SysClkSrc /= 128; }
        else if (global_uint8AHBPres == MRCC_AHB_DIV_BY_256) { local_uint64SysClkSrc /= 256; }
        else if (global_uint8AHBPres == MRCC_AHB_DIV_BY_512) { local_uint64SysClkSrc /= 512; }
        else {}
 
        /* if bus is one of the APB buses, calculate its clk */
        switch (copy_enuBus)
        {
            case SYS_BUS_APB1:
            case SYS_BUS_APB2:
                if (global_uint8AHBPres == MRCC_APB_DIV_BY_1) {}
                else if (global_uint8AHBPres == MRCC_APB_DIV_BY_2)   { local_uint64SysClkSrc /=  2; }
                else if (global_uint8AHBPres == MRCC_APB_DIV_BY_4)   { local_uint64SysClkSrc /=  4; }
                else if (global_uint8AHBPres == MRCC_APB_DIV_BY_8)   { local_uint64SysClkSrc /=  8; }
                else if (global_uint8AHBPres == MRCC_APB_DIV_BY_16)  { local_uint64SysClkSrc /= 16; }
                else {}
            break;
        }
 
        /* return bus clk freq through passed ptr */
        (*ptr_uint64BusClkFreq) = local_uint64SysClkSrc;
    }
